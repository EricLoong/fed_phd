...
from standalone.moon_ddim.moon_api import MOONAPI
from standalone.moon_ddim.moon_trainer import MOONTrainer
...
def add_args(parser):
    ...
    # Moon arguments
    parser.add_argument('--contrastive_loss_weight', type=float, default=0.1, help='Proximal term scale')
    return parser
...
def setup_trainer(args, diffusion_model, fid_scorer, inception_scorer, ddim_samplers, logger):
    ...
    trainer = MOONTrainer(args=args, logger=logger,
...
if __name__ == "__main__":
    ...
    if args.dataset == "celeba":
        ...
        raise ValueError(f"Dataset {args.dataset} not supported")
    MoonAPI = MOONAPI(data_info, device, args, global_model_trainer, logger=logger)
    final_global_model = MoonAPI.train()