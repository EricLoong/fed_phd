...
def setup_trainer(args, diffusion_model, fid_scorer, inception_scorer, ddim_samplers, logger):
    ...
    trainer.global_control_variate = {k: torch.zeros_like(v) for k, v in diffusion_model.state_dict().items()}
    return trainer

if __name__ == "__main__":
    torch.cuda.empty_cache()
    parser = add_args(argparse.ArgumentParser(description="SCAFFOLD-standalone"))
    ...
    # Define identity to reflect SCAFFOLD usage
    args.identity = "scaffold" + "-" + data_partition
    ...
    ddim_samplers = setup_ddim_sampler(args, diffusion_model)
